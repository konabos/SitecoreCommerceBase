...
gulp.task('Stop-Local-IIS', function (callback) {
    exec('Powershell.exe -executionpolicy UnRestricted   iisreset /timeout:0 /stop', function (err, stdout) {
        console.log(stdout);
        callback(err);
    });
});
...
gulp.task('Start-Local-IIS', function (callback) {
    exec('Powershell.exe -executionpolicy UnRestricted   iisreset /timeout:0 /start', function (err, stdout) {
        console.log(stdout);
        callback(err);
    });
});
